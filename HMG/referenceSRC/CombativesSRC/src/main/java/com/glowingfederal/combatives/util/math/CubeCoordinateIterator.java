...
public class CubeCoordinateIterator {
    ...
    public CubeCoordinateIterator(int startX, int startY, int startZ, int endX, int endY, int endZ) {
        this.startX = startX;
        this.startY = startY;
        this.startZ = startZ;
        this.xWidth = endX - startX + 1;
        this.yHeight = endY - startY + 1;
        this.zWidth = endZ - startZ + 1;
        this.totalAmount = this.xWidth * this.yHeight * this.zWidth;
    }

    public boolean hasNext() {
        if (this.currentAmount == this.totalAmount) return false;
    ...
    public int getX() { return this.startX + this.x; }
    public int getY() { return this.startY + this.y; }
    public int getZ() { return this.startZ + this.z; }

    public int numBoundariesTouched() {
        int count = 0;
        if (this.x == 0 || this.x == this.xWidth - 1) ++count;
        if (this.y == 0 || this.y == this.yHeight - 1) ++count;
        if (this.z == 0 || this.z == this.zWidth - 1) ++count;
        return count;
    }
}
