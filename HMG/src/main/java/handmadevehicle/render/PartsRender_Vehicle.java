...
public class PartsRender_Vehicle extends PartsRender {
	...
	public void partSidentification(Object... data){
		FMLClientHandler.instance().getWorldClient().theProfiler.startSection("PartSidentification_Vehicle");
		float flame;
	...
	public boolean partModel_render(HMGGunParts parts, GunState state, float flame, int remainbullets, HMGGunParts_Motion_PosAndRotation OffsetAndRotation){
		...
		if(parts instanceof HMVVehicleParts) {
			if(((HMVVehicleParts) parts).linkedTurretID >= 0) {
				...
				if (((HMVVehicleParts) parts).isTurretParts && parts.hasbaseYawInfo) {
					...
					if (baserotationCenterAndRotation != null) {
						if (!baserotationCenterAndRotation.renderOnOff) return false;
			...
			if(((HMVVehicleParts) parts).somethingMotionKey != null) {
				...
				if (yawLadderInfo != null) {
					if (!yawLadderInfo.renderOnOff) return false;
				...
				if (pitchLadderInfo != null) {
					if (!pitchLadderInfo.renderOnOff) return false;
				...
				if (rollLadderInfo != null) {
					if (!rollLadderInfo.renderOnOff) return false;
				...
				if (throttleInfo != null) {
					if (!throttleInfo.renderOnOff) return false;
				...
				if (yawInfo != null) {
					if (!yawInfo.renderOnOff) return false;
				...
				if (pitchInfo != null) {
					if (!pitchInfo.renderOnOff) return false;
				...
				if (rollInfo != null) {
					if (!rollInfo.renderOnOff) return false;
				...
				if (gearInfo != null) {
					if (!gearInfo.renderOnOff) return false;
				...
				if (flapInfo != null) {
					if (!flapInfo.renderOnOff) return false;
				...
				if (motionInfox != null) {
					if (!motionInfox.renderOnOff) return false;
				...
				if (motionInfoy != null) {
					if (!motionInfoy.renderOnOff) return false;
				...
				if (motionInfoz != null) {
					if (!motionInfoz.renderOnOff) return false;
				...
				if (brakeMotion != null) {
					if (!brakeMotion.renderOnOff) return false;
				...
				if (globalMotionInfox != null) {
					if (!globalMotionInfox.renderOnOff) return false;
				...
				if (globalMotionInfoy != null) {
					if (!globalMotionInfoy.renderOnOff) return false;
				...
				if (globalMotionInfoz != null) {
					if (!globalMotionInfoz.renderOnOff) return false;
				...
				if (ALTInfo != null) {
					if (!ALTInfo.renderOnOff) return false;
			...
			if (parts.isbullet) {
				...
				boolean flag = super.partModel_render(parts, state, flame, remainbullets, OffsetAndRotation);
				if (!flag && ((HMVVehicleParts) parts).isTurret_linkedGunMount &&
...
//					}
//				}
				if(parts.trackMotionKeyCache == null){
					parts.trackMotionKeyCache = new HMGGunParts_Motions[parts.trackPieceCount];

					if(parts.trackPositions.motionArray == null){
						parts.trackPositions.init();
					}
					for (int i = 0; i < parts.trackPieceCount; i++) {
//						System.out.println("cacheID" + i);
						parts.trackMotionKeyCache[i] = new HMGGunParts_Motions();
						HMGGunParts_Motion tempStart = parts.trackPositions.getmotionobject(i);
						int startMotionID = parts.trackPositions.currentMotionID;
//						System.out.println("cacheID\tStart\t" + startMotionID);
//						System.out.println("cacheID\tStart\t" + tempStart);
						HMGGunParts_Motion tempEnd = parts.trackPositions.getmotionobject(i + 1);
//						System.out.println("cacheID\t End \t" + parts.trackPositions.currentMotionID);
//						System.out.println("cacheID\t End \t" + tempEnd);
						if(tempStart != null) parts.trackMotionKeyCache[i].addmotion(tempStart);
						if(startMotionID == parts.trackPositions.currentMotionID)continue;
						if(startMotionID+1 == parts.trackPositions.currentMotionID){
							if(tempEnd != null)parts.trackMotionKeyCache[i].addmotion(tempEnd);
							continue;
						}
						for (int cnt = startMotionID; cnt <= parts.trackPositions.currentMotionID; cnt++) {
							HMGGunParts_Motion tempInter = parts.trackPositions.motionArray[cnt];
							if(tempInter != null)
								parts.trackMotionKeyCache[i].addmotion(tempInter);
						}
					}
				}
...
//					HMGGunParts_Motion_PosAndRotation trackoffset = parts.getTrackPositions(i + flame);
					...
					HMGGunParts_Motion_PosAndRotation trackoffset = parts.trackMotionKeyCache[i].getpartsMotion(i + flame);
