	...
	public void onUpdateMoveHelper()
	{
		...
		if(baseLogic.riddenByEntities[0] instanceof EntityLiving && !baseLogic.riddenByEntities[0].isDead) {
			...
			if (!entity.worldObj.isRemote) {
				...
				this.setToLogic(this.baseLogic.bodyrotationYaw, f,distToFinal);//TODO �����͕K���ŏI�ړI�n�܂łŌv�Z����悤�ɕς��悤
			}
		}
	}
...
//			baseLogic.seatObjects[0].gunTrigger[0] = false;
//			baseLogic.seatObjects[0].gunTrigger[1] = false;
//			Vector3d targetPos = getSeeingPosition(target);
//			if(baseLogic.seatObjects[0].subWeapon != null) {
//				subWeapon = baseLogic.seatObjects[0].subWeapon;
//			}
//			if(weaponChangeCool < 0 && rand.nextInt(30) == 0){
//				T_useMain_F_useSub = !T_useMain_F_useSub;
//				weaponChangeCool = 100;
//			}else {
//				weaponChangeCool--;
//			}
//			if(baseLogic.seatObjects[0].mainWeapon != null) {
//				mainWeapon = baseLogic.seatObjects[0].mainWeapon[baseLogic.seatObjects[0].currentWeaponMode];
//				if (!mainWeapon.hasWaitToReadyWeapon()){
//					baseLogic.seatObjects[0].currentWeaponMode++;
//					if (baseLogic.seatObjects[0].currentWeaponMode >= baseLogic.seatObjects[0].mainWeapon.length) {
//						baseLogic.seatObjects[0].currentWeaponMode = 0;
//					}
//					mainWeapon = baseLogic.seatObjects[0].mainWeapon[baseLogic.seatObjects[0].currentWeaponMode];
//				}
//			}
//			WeaponCategory currentWeapon = T_useMain_F_useSub ? mainWeapon : subWeapon;
//			if(currentWeapon == null || currentWeapon.getCriterionTurret() == null || currentWeapon.getCriterionTurret().gunItem == null || !currentWeapon.hasWaitToReadyWeapon()){
//				T_useMain_F_useSub = !T_useMain_F_useSub;
//				currentWeapon = T_useMain_F_useSub ? mainWeapon : subWeapon;
//			}
//			if(currentWeapon == null || currentWeapon.getCriterionTurret() == null || currentWeapon.getCriterionTurret().gunItem == null || !currentWeapon.hasWaitToReadyWeapon()){
//				currentWeapon = null;
//			}
//			boolean noWeapon = currentWeapon == null;
			if((pilot).getAttackTarget() != null && !(pilot).getAttackTarget().isDead){
//				double targetyaw = 0;
...
//				if(mainWeapon.getCriterionTurret() != null) thisPos = mainWeapon.getCriterionTurret().forAim_getCannonPosGlobal();
//				courseVec.sub(thisPos);
//				double terminalSpeed = 0;
//				if(currentWeapon != null && currentWeapon.getCriterionTurret() != null){
//					terminalSpeed = currentWeapon.getSpeed();
//				}
//				if(!noWeapon){
//					if(prefab_vehicle.type_F_Plane_T_Heli && currentWeapon.getCriterionTurret() != null){
//						Vector3d motionVec = new Vector3d(target.motionX - baseLogic.mc_Entity.motionX,target.motionY - baseLogic.mc_Entity.motionY,target.motionZ - baseLogic.mc_Entity.motionZ);
//
////						if (terminalSpeed != 0) {
...
////				if(currentMainWeapon != null)System.out.println("" + currentMainWeapon.prefab_weaponCategory.name);
//				if(noWeapon){
//					targetpitch = prefab_vehicle.type_F_Plane_T_Heli?prefab_vehicle.cruiseNoseDown:0;
//				}else
//				if(currentWeapon.getCriterionTurret() != null && currentWeapon.getCriterionTurret().gunItem != null){
//					courseVec.add(thisPos);
//
//					Vector3d cannonPos = currentWeapon.getCriterionTurret().forAim_getCannonPosGlobal();
//
//					double[] elevationTemp = (CalculateGunElevationAngle(
//							cannonPos.x,
//							cannonPos.y,
//							cannonPos.z,
//
//							courseVec.x,
//							courseVec.y,
//							courseVec.z, currentWeapon.getCriterionTurret().gunItem.gunInfo.gravity * (float) HandmadeGunsCore.cfg_defgravitycof, terminalSpeed - baseLogic.localMotionVec.z));
//					targetpitch = -elevationTemp[0];
//					canAttack = elevationTemp[2] != -1;
//					if(!canAttack)targetpitch = -10;
//				}
//				if(currentWeapon != null && currentWeapon.getCriterionTurret() != null &&
//						currentWeapon.getCriterionTurret().getCurrentGuninfo().canlock && currentWeapon.target == null){
//					if(ATG){
...
//
//				if(currentWeapon != null && currentWeapon.getCriterionTurret() != null &&
//						currentWeapon.getCriterionTurret().getCurrentGuninfo().canlock && currentWeapon.target != null && currentWeapon.target != target){
//					canAttack = false;
//				}
////				System.out.println("debug" + baseLogic.localMotionVec.z);
////				System.out.println("debug" + terminalSpeed);
////				System.out.println("debug" + targetpitch);
////				System.out.println("debug" + T_useMain_F_useSub);
...
//								angularDifferencePitch < prefab_vehicle.pitchsighwidthmax && canAttack;
//				if(!ATG) {
...
//						baseLogic.server_easyMode_yawTarget = targetyaw;
//						baseLogic.server_easyMode_pitchTarget = targetpitch;
//						if (abs(angularDifferenceYaw) > 5) {
//							double temp = (abs(angularDifferenceYaw)-5)/45;
//							if(temp > 1)temp = 1;
//							baseLogic.server_easyMode_pitchTarget = targetpitch*(1-temp);
...
//							baseLogic.seatObjects[0].gunTrigger[0] = true;
//							baseLogic.seatObjects[0].gunTrigger[1] = true;
				...
				switch (mode) {
					...
					case Go://follow
						...
						if(((Hasmode) pilot).getMoveToPos() != null){
							moveToPos = new double[]{((Hasmode) pilot).getMoveToPos().x,
									((Hasmode) pilot).getMoveToPos().y,
									((Hasmode) pilot).getMoveToPos().z};
						}else {
							moveToPos = ((Hasmode) pilot).getTargetpos();
						}
					break;
					...
					case Land: {
						...
						if (pilot instanceof Hasmode)
							moveToPos = ((Hasmode) pilot).getTargetpos();
						if (moveToPos == null)
							moveToPos = new double[]{baseLogic.mc_Entity.posX, baseLogic.mc_Entity.posY, baseLogic.mc_Entity.posZ};
						alt = (float) (baseLogic.mc_Entity.posY - moveToPos[1]);
						Vector3d courseVec = new Vector3d(moveToPos);
...
//						System.out.println("" + angletocourse);
				...
				if(moveToPos == null)moveToPos = new double[]{baseLogic.mc_Entity.posX, baseLogic.mc_Entity.posY, baseLogic.mc_Entity.posZ};
//				System.out.println("" + moveToPos[0] + " , " +moveToPos[1]+" , " + moveToPos[2]);
				Vector3d motionvec = new Vector3d(baseLogic.mc_Entity.motionX, baseLogic.mc_Entity.motionY, baseLogic.mc_Entity.motionZ);
				if (motionvec.length() > 0.1) motionvec.normalize();
				Vector3d courseVec = new Vector3d(moveToPos);
				courseVec.sub(new Vector3d(baseLogic.mc_Entity.posX, courseVec.y, baseLogic.mc_Entity.posZ));
