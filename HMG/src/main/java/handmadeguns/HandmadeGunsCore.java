...
public class HandmadeGunsCore {
	...
	public static boolean cfg_ClientCamera_AngelicaCompatEnabled = true;
	public static boolean cfg_ClientCamera_DisableAngelicaViewBobbing = true;
	public static boolean cfg_ClientCamera_DisableAngelicaDynamicFovWhenHmgFovEnabled = true;
	public static boolean cfg_ClientCamera_PreferHmgBobbingWhenAngelicaLoaded = true;
	public static boolean cfg_ClientCamera_UseLegacyRecoilWhenCameraDisabled = true;
	...
	public void preInit(FMLPreInitializationEvent pEvent) {
		...
		cfg_ClientCamera_MasterEnabled = lconf.get(cameraCategory, "enableOverdriveCamera", true, "Master switch for visual client-only first-person camera features. Gameplay recoil remains active; when disabled, legacy visual recoil is preserved.").getBoolean(true);
		cfg_ClientCamera_AngelicaCompatEnabled = lconf.get(cameraCategory, "enableAngelicaCompat", true, "When Angelica is loaded, let HMG own first-person bob/FOV camera behavior to avoid stacked jitter.").getBoolean(true);
		cfg_ClientCamera_DisableAngelicaViewBobbing = lconf.get(cameraCategory, "disableAngelicaViewBobbingCompat", true, "Compat only: force vanilla/Angelica view bobbing off while HMG Overdrive provides procedural step bob.").getBoolean(true);
		cfg_ClientCamera_DisableAngelicaDynamicFovWhenHmgFovEnabled = lconf.get(cameraCategory, "disableAngelicaDynamicFovCompat", true, "Compat only: force Angelica dynamic FOV to NONE while HMG FOV inertia is active.").getBoolean(true);
		cfg_ClientCamera_PreferHmgBobbingWhenAngelicaLoaded = true;
		cfg_ClientCamera_UseLegacyRecoilWhenCameraDisabled = lconf.get(cameraCategory, "useLegacyRecoilWhenCameraDisabled", true, "When Overdrive visual camera is disabled, keep the previous RenderTickSmoothing recoil behavior exactly.").getBoolean(true);
		cfg_ClientCamera_DebugCamera = lconf.get(cameraCategory, "debugCamera", false, "Print soft-fail diagnostics for camera compatibility hooks.").getBoolean(false);
