...
public class MQO_MetasequoiaObject implements IModelCustom_HMG
{
	...
	public void checkMinMax(HMGVertex v)
	{
		...
		for (MQO_GroupObject groupObject : groupObjects)
		{
			if (groupObject.faces_PerMat[0].size() > 0)
			{
				for (Object obj : groupObject.faces_PerMat[0])
				{
					...
					{
						HMGVertex v1 = face.vertices[i*3 + 0];
						HMGVertex v2 = face.vertices[i*3 + 1];
						HMGVertex v3 = face.vertices[i*3 + 2];
		...
		try
		{
			...
			while ((currentLine = reader.readLine()) != null)
			{
				...
				if(isValidGroupObjectLine(currentLine))
				{
					...
					group.faces_PerMat = new ArrayList[materials == null?1:materials.length];
					Arrays.fill(group.faces_PerMat, new ArrayList<MQO_Face>());

					for(int id = 0; id < group.faces_PerMat.length; id++){
						group.faces_PerMat[id] = new ArrayList<MQO_Face>();
					}
							...
							while ((currentLine = reader.readLine()) != null)
							{
								...
								if(s.length > 2)
								{
									if(Integer.valueOf(s[0]) >= 3)
									{
										...
										{
											group.faces_PerMat[face.faceMat].add(face);
					...
					for(int id = 0; id < group.faces_PerMat.length; id++){
						this.faceNum   += group.faces_PerMat[id].size();
					}
					this.vertices.clear();
	...
	{
		for(int id = 0; id < group.faces_PerMat.length; id++){
			for(MQO_Face f : (ArrayList<MQO_Face>)group.faces_PerMat[id])
			{
				f.vertexNormals = new HMGVertex[f.verticesID.length];
				for(int i=0; i<f.verticesID.length; i++)
				{
					HMGVertex vn = getVerticesNormalFromFace(f.faceNormal, f.verticesID[i], group, (float)facet);
	...
	private HMGVertex getVerticesNormalFromFace(HMGVertex faceNormal, int verticesID, MQO_GroupObject group, float facet)
	{
		HMGVertex v = new HMGVertex(0,0,0);

		for(int i = 0; i < group.faces_PerMat.length; i++) {
			for(MQO_Face f : (ArrayList<MQO_Face>)group.faces_PerMat[i]) {
