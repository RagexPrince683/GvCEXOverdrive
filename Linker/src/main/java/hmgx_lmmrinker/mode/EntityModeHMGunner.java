...
//        System.out.println("debug");
	...
	public void onUpdate(int pMode) {
		...
		if(owner.getMaidMasterEntity() != null && !owner.isFreedom() && !owner.isMaidWait()){
			EntityPlayer master = owner.getMaidMasterEntity();
			if(master.ridingEntity instanceof EntityVehicle || master.ridingEntity instanceof EntityDummy_rider){
				BaseLogic baseLogic;
				if(master.ridingEntity instanceof EntityVehicle){
					baseLogic = ((EntityVehicle) master.ridingEntity).getBaseLogic();
				}else {
					baseLogic = ((EntityDummy_rider) master.ridingEntity).linkedBaseLogic;
				}
				baseLogic.pickupEntity(this.owner,1,true);
			}else if(owner.ridingEntity instanceof EntityDummy_rider){
				BaseLogic baseLogic = ((EntityDummy_rider) owner.ridingEntity).linkedBaseLogic;
				baseLogic.disMountEntity(owner);
			}
		}
	}
}
