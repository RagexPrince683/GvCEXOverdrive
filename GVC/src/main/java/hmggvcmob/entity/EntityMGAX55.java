...
public class EntityMGAX55 extends Entity implements I_SPdamageHandle {
	...
	public SeatObject[] seatObject = new SeatObject[6];
...
//        }
	...
	public boolean attackEntityFrom_with_Info(MovingObjectPosition movingObjectPosition, DamageSource source, float level){
		...
		if(source instanceof EntityDamageSourceIndirect){
			...
			temparomor -= entity.motionX * entity.motionX + entity.motionY * entity.motionY + entity.motionZ * entity.motionZ;
		}
...
//    }
	...
	void control_Pilot(){
		...
		legSneak_CTRL = HMV_Proxy.iszooming();
		if(!onGround){
			legSneak_CTRL = false;
		}
...
//            }
			...
			if(((EntityLiving) pilot).getNavigator() instanceof ModifiedPathNavigater)navigator.setPath(((EntityLiving) pilot).getNavigator().getPath(),((ModifiedPathNavigater) ((EntityLiving) pilot).getNavigator()).getSpeed());
		}
		packetMGControl.w = w;
		packetMGControl.a = a;
		packetMGControl.s = s;
	...
	void FireRail(){
		...
		PacketSpawnParticle flash = new PacketSpawnParticle(
				...
				bullet.posZ + bullet.motionZ*0.8,bodyrotationYaw + headrotationYaw, RailGUNrotationPitch,0,"RailMuzzleFlash",true);
		flash.fuse = 3;
		flash.scale = 100;
		flash.id = 100;
...
//    }
	...
	{
		return 9 - legSneak_State_Progress/10d;
	}
...
//    }
	...
	public void collideWithNearbyEntities()
	{
		...
		if (list != null && !list.isEmpty()) for (Object o : list) {
			...
			if (entity instanceof IGVCmob && entity.canBePushed() && !isRidingEntity(entity) && this.riddenByEntity == null) {
